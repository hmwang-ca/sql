...
-- latest price per vendor-product (avoid counting old prices)
latest_vendor_products AS (
...
-- CROSS JOIN: every vendor-product to every customer
exploded AS (
  SELECT
    lvp.vendor_id,
    lvp.product_id,
    lvp.original_price,
    c.customer_id
  FROM latest_vendor_products lvp
...
SELECT
  v.vendor_name,
  p.product_name,
  SUM(5 * e.original_price) AS total_revenue_if_each_customer_buys_5
FROM exploded e
JOIN vendor  v ON v.vendor_id  = e.vendor_id
